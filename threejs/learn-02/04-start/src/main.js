...
gui.add(params, 'isLight').name('是否开启灯光').onChange(val => {
  if(val) {
    renderer.toneMappingExposure = val
  } else {
    renderer.toneMappingExposure = 0.1
  }
}) //只要改变环境灯光就可以了，修改环境灯光只需要设置色调映射就可以了
...
snapFolder.add(params, 'translateSnapNum', {
  '不固定': null,
  1: 1,
  0.1: 0.1,
  10: 10
...
.name('固定位移设置')
.onChange(() => {
  tControls.setTranslationSnap(tControls.translateSnapNum)
})

snapFolder.add(params, 'rotateSnapNum', 0, 1)
.step(0.1).name('旋转')
.onChange(() => {
  tControls.setRotationSnap(params.rotateSnapNum * Math.PI * 2)
})

snapFolder.add(params, 'scaleSnapNum', 0, 10)
.step(0.1).name('缩放')
.onChange(() => {
  tControls.setScaleSnap(params.scaleSnapNum)
})

snapFolder.add(params, 'isClampGroup')
.name('是否吸附到地面')
