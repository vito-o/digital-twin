...
materialNode.colorNode = Nodes.texture(uvTexture).add(
  Nodes.timerLocal().sin(),
  Nodes.uv()
  // Nodes.uv().mul(Nodes.timerLocal().sin())
  // Nodes.uv().mul(Nodes.timerLocal().mul(0.1).remainder(1))
);
materialNode.opacityNode = Nodes.texture(
  opacityTexture,
  Nodes.uv().add(Nodes.vec2(0, Nodes.timerLocal()))
);
// materialNode.alphaTestNode = Nodes.timerLocal();
